...
export function Reports() {
    ...
    async function loadReportsData() {
      try {
        ...
        const [kpiData, categoryData, monthlyData, vendorsData] = await Promise.all([
          ...
          apiFetch('/analytics/monthly-spend'),
  ...
  const totalSpendNumeric = spendByCategory.reduce((acc, curr) => acc + (Number(curr.value) || 0), 0);
                ...
                {topVendors.length === 0 ? (
                  ...
                  topVendors.map((vendor, index) => (
                    <tr key={index}>
                      ...
                      <td className="px-4 py-3 text-zinc-600">{Number(vendor.spend).toLocaleString('en-IN')}</td>
